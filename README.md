# project-laravel 
pastikan sudah menginstall laragon dan composer
1. jalankan laragon, lalu klik/buka terminal
![laragon](foto/laragon.png)
2. Download laravel seperti dibawah ini
ketik prompt 
composer create-project --prefer-dist laravel/laravel nama_project
tunggu sampai proses selesai
![install](foto/install.png)
4. dan apabila success
![success](foto/success.png)
5. buka laravel dan ketik (cd nama_Project)
buka text editor dan ketik (cd .)
![prompt](foto/prompt.png)
6. untuk menjalankan project ketik prompt (php artisan serve)
![terminal](foto/terminal.png)
7. dan cek. laravel sudah berjalan.
![berjalan](foto/berjalan.png)
